   ...
   var path = "/images/"; //定义图片路径
    ...
    lastBuild: function() {
        ...
        if (this.config.contentType == 1) {
            ...
            G("dialogBody").innerHTML = '<div style="width:100%;height:100%;background: url(../images/popup/s_bg.jpg) no-repeat right bottom;"><div style="margin:10px;font-size:14px;">' + this.info.contentHtml + '</div></div>';
          
        }
        else if (this.config.contentType == 3) {
            //选择类型 修改者：Alpha 修改时间：081202
            G("dialogBody").innerHTML = '<div style="width:100%;height:100%;background: url(../images/popup/p_bg.jpg) no-repeat right bottom;"><div style="margin:10px;font-size:14px;">' + this.info.confirmCon + '</div><div style="margin:20px;text-align:center;"><input id="dialogOk" type="button" value="  确定  "/>&nbsp;<input id="dialogCancel" type="button" value="  取消  "/></div></div>';
            Event.observe(G('dialogOk'), "click", this.forCallback.bindAsEventListener(this), false);
            Event.observe(G('dialogCancel'), "click", this.close.bindAsEventListener(this), false)
        }
        else if (this.config.contentType == 4) {
            //警示类型 修改者：Alpha 修改时间：081202
            G("dialogBody").innerHTML = '<div style="width:100%;height:100%;background: url(../images/popup/w_bg.jpg) no-repeat right bottom;"><div style="margin:10px;font-size:14px;">' + this.info.alertCon + '</div><div style="margin:20px;text-align:center;"><input id="dialogYES" type="button" value="  确定  "/></div></div>';
