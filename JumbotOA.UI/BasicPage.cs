...
namespace JumbotOA.UI
{
    ...
    public class BasicPage : System.Web.UI.Page
    {
        ...
        {
            JumbotOA.Utils.Cookie.Del("oa_user");
            return true;
        }
        /// <summary>
        /// 管理菜单--管理员
        ...
        {
            return JumbotOA.Utils.Validator.StrToInt(s, t);
        }
        ...
        {
            return JumbotOA.Utils.Validator.StrToInt(s, 0).ToString();
        }
        public void DownloadFile(string _filePath)
        {
            ...
            else if (!JumbotOA.Utils.DirFile.FileExists(_filePath))
            {
                Response.Write("指定的文件不存在,请通知管理员");
            }
            else
            {
                success = JumbotOA.Utils.DirFile.DownloadFile(Request, Response, Server.MapPath(_filePath), 1024000);
                if (!success)
                {
                    Response.Redirect(_filePath);
                }
        ...
        public string getvalue(int i)
        {
            ...
            {
                if(i==1)
                    powerid = JumbotOA.Utils.Cookie.GetValue("oa_user", "id").ToString();
                 else if(i==2)
                    powerid = JumbotOA.Utils.Cookie.GetValue("oa_user", "name").ToString();
                   else if(i==3)
                    powerid = JumbotOA.Utils.Cookie.GetValue("oa_user", "ip").ToString();
                 else if(i==4)
                    powerid = JumbotOA.Utils.Cookie.GetValue("oa_user", "Powerid").ToString();
            }
            return powerid;
        }
    }
